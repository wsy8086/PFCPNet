...
from Amodel.ridnet.TestCode.code.model.ridnet import  RIDNET
# from Amodel.CBDnet.cbdnet import Network
from Amodel.GrencNet.GrencNet import DN as grenc
from Amodel.casapunet.CasaPuNet import Network
from Amodel.deamnet.DeamNet import Deam
import torch
...
# checkpoint = torch.load(r'F:\Pan\results\Amodel\CBDnet\checkpoint.pth.tar', map_location=device)
...
# w = torch.load(r'F:\Pan\results\Amodel\dncnn\dncnn_color_blind.pth')
# model.load_state_dict(w)
# model.eval()

# checkpoint = torch.load(r'F:\Pan\results\Amodel\ridnet\experiment\ridnet.pt')
# model = RIDNET().to(device)
# model = torch.nn.DataParallel(model)
# model.load_state_dict(checkpoint['state_dict'])
# model.eval()

# checkpoint = torch.load(r'F:\Pan\results\Amodel\GrencNet\checkpoint_39.45_0.9188.pth', map_location=device)
# model = grenc().to(device)
# model.load_state_dict(checkpoint)
# model.eval()
#
# model = Network()
# model =torch.nn.DataParallel(model).cuda()
# weight = torch.load(r'F:\Pan\results\Amodel\casapunet\checkpoint.pth.tar')
# model.load_state_dict(weight['state_dict'])
# model.eval()

# model = Deam(True)
# model =torch.nn.DataParallel(model).cuda()
# weight = torch.load(r'F:\Pan\results\Amodel\deamnet\Deam_models\Real.pth')
# model.load_state_dict(weight)
# model.eval()


image_list = glob.glob('F:/Pan/results/visual/real/IM777' + "/*")
...
for i in range(img_nums):
    ...
    if save_denoised_image:
        ...
        io.imsave(os.path.join('F:/work/XIAO/ROI/ROI/IMGVIS/noise/x4/noise_01/1/cfnet.png'), im_denoise)
   #  for i in range(64):
